################################################################################
# Run Kraken2 with GTDB r202 database on all baboon calculus samples
#
...
#### SAMPLES ###################################################################
SAMPLES = {}
for sample in glob("/mnt/archgen/microbiome_calculus/strep_clades/04-analysis/kraken2_gtdb_r202/input_nhp_baboon/*.gz"):
...
rule all:
    input: 
        expand("out_nhp_baboon/{sample}.kraken2.gtdb.output.tsv", sample=SAMPLES.keys()),
        expand("out_nhp_baboon/{sample}.kraken2.gtdb.report_mpa.tsv", sample=SAMPLES.keys())

rule kraken_classify:
    output:
        outfmt = "out_nhp_baboon/{sample}.kraken2.gtdb.output.tsv",
        repfmt = "out_nhp_baboon/{sample}.kraken2.gtdb.report_mpa.tsv"
