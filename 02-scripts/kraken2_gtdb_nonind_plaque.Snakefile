################################################################################
# Run Kraken2 with GTDB r202 database on all human non-industrial plaque samples
#
...
rule all:
    input: 
        expand("out_nonind_plaque/{sample}.kraken2.gtdb.output.tsv", sample=SAMPLES.keys()),
        expand("out_nonind_plaque/{sample}.kraken2.gtdb.report_mpa.tsv", sample=SAMPLES.keys())

rule kraken_classify:
    output:
        outfmt = "out_nonind_plaque/{sample}.kraken2.gtdb.output.tsv",
        repfmt = "out_nonind_plaque/{sample}.kraken2.gtdb.report_mpa.tsv"
