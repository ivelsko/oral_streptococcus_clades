...
#### SAMPLES ###################################################################
SAMPLES, = glob_wildcards("/mnt/archgen/microbiome_calculus/strep_clades/04-analysis/kraken2_gtdb_r202/input_anc_calc/{sample}.unmapped.fastq.gz")
...
rule MetaPhlAn3:
    input: 
        expand("anc/metaphlan3/{sample}.metaphlan.profile.txt", sample=SAMPLES)

rule count_nreads:
    output:
        "anc/metaphlan3/{sample}.nreads"
    message: "Count the number of reads: {wildcards.sample}"
    params:
        fastq = "/mnt/archgen/microbiome_calculus/strep_clades/04-analysis/kraken2_gtdb_r202/input_anc_calc/{sample}.unmapped.fastq.gz"
    shell:
        "bioawk -c fastx 'END{{print NR}}' {params.fastq} > {output}"

checkpoint summarise_nreads:
    input:
        expand("anc/metaphlan3/{sample}.nreads", sample=SAMPLES)
    output:
        "anc/metaphlan3/summary_nreads.tsv"
...
rule bowtie2:
    output:
        "anc/metaphlan3/{sample}.metaphlan.bwt2.sam"
...
rule metaphlan3:
    input:
        bowtie2 = "anc/metaphlan3/{sample}.metaphlan.bwt2.sam",
        nreads = "anc/metaphlan3/summary_nreads.tsv"
    output: 
        "anc/metaphlan3/{sample}.metaphlan.profile.txt"
