...
# S. sanguinis genome with the highest mapping from output of 
...
rule all:
     input: 
         expand("{sample}-sanguinis.mapped_rmdup.bam", sample=SAMPLES.keys()),
         expand("{sample}-sanguinis.mapped_rmdup.bam.bai", sample=SAMPLES.keys()),
         expand("{sample}-sanguinis.mapped_rmdup.reads.txt", sample=SAMPLES.keys()),
         expand("{sample}-sanguinis.mapped_rmdup.cov", sample=SAMPLES.keys()),
         expand("{sample}-sanguinis.mapped_rmdup.d.cov", sample=SAMPLES.keys())

rule bwa_aln:
     output:
         temp("{sample}-sanguinis.sai")
     message: "Align sample {wildcards.sample} against S. sanguinis genome using BWA aln"
     params: 
...
rule bwa_samse_aln:
    input:
        "{sample}-sanguinis.sai"
    output:
        bam = "{sample}-sanguinis.mapped.bam",
        reads = temp("{sample}-sanguinis.mapped.reads.txt")
...
rule samtools_rmdup_mem:
    input:
        "{sample}-sanguinis.mapped.bam"
    output:
        bam = "{sample}-sanguinis.mapped_rmdup.bam",
        reads = "{sample}-sanguinis.mapped_rmdup.reads.txt"
...
rule samtools_index_mem_rmdup:
    input:
        "{sample}-sanguinis.mapped_rmdup.bam"
    output:
        "{sample}-sanguinis.mapped_rmdup.bam.bai"
...
rule bedtools_coverage_rmdup:
    input:
        "{sample}-sanguinis.mapped_rmdup.bam"
    output:
        "{sample}-sanguinis.mapped_rmdup.cov"
...
rule bed_cov_rmdup_d:
    input:
        "{sample}-sanguinis.mapped_rmdup.bam"
    output:
        "{sample}-sanguinis.mapped_rmdup.d.cov"
    message: "Calculate depth/breadth of coverage of {wildcards.sample}"
    params:
    shell:
        """
