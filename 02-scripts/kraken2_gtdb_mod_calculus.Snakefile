################################################################################
# Run Kraken2 with GTDB r202 database on all human modern calculus samples
#
...
#### SAMPLES ###################################################################
SAMPLES = {}
for sample in glob("/mnt/archgen/microbiome_calculus/strep_clades/04-analysis/kraken2_gtdb_r202/input_mod_calc/*.gz"):
...
rule all:
    input: 
        expand("out_mod_calc/{sample}.kraken2.gtdb.output.tsv", sample=SAMPLES.keys()),
        expand("out_mod_calc/{sample}.kraken2.gtdb.report_mpa.tsv", sample=SAMPLES.keys())

rule kraken_classify:
    output:
        outfmt = "out_mod_calc/{sample}.kraken2.gtdb.output.tsv",
        repfmt = "out_mod_calc/{sample}.kraken2.gtdb.report_mpa.tsv"
