...
rule all:
    input: 
        expand("{sample}-sinensis_sanguinis_rep.mapped.bam.bai", sample=SAMPLES.keys()),
...
rule samtools_rmdup_mem:
    ...
    message: "Remove duplicate mapped reads for sample {wildcards.sample} against gene cat'd genes"
    params:
    shell:
        """
...
rule samtools_index_mem:
    input:
        "{sample}-sinensis_sanguinis_rep.mapped.bam"
    output:
        "{sample}-sinensis_sanguinis_rep.mapped.bam.bai"
...
rule bedtools_coverage_rmdup:
    ...
    message: "Calculate depth/breadth of coverage of {wildcards.sample} mapped against cat'd genes"
    params:
    shell:
        """
