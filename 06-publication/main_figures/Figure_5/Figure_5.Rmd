...
ip_points <- aardvark%>%
  ...
  mutate(Library_ID = fct_relevel(Library_ID, sample_order_rs)) %>%
  mutate(Full = fct_relevel(Full, "Calculus - Industrial","Plaque - Non-industrial","Plaque - Industrial","Buccal mucosa - Non-industrial","Buccal mucosa - Industrial","Saliva - Non-industrial","Saliva - Industrial")) %>%
  ggplot(., aes(x = Library_ID, y = Percent, shape = Genus, color = Genus, size = Genus)) +
         geom_point() + 
         theme_minimal(base_size = 10) +
         scale_color_manual(values = clade_colors) + # c("#fee090","#a50026")
         scale_shape_manual(values = c(16,18,17)) +
         scale_size_manual(values = c(2.5, 3, 2.5)) +
         theme(
...
strep_clades_pct <- mod_oral_raw_sp %>%
  # remove poorly-preserved samples
  select(-matches(poor_samples %>% str_c(collapse = "|"))) %>%
  # filter(str_detect(Species, "Streptococcus")) %>%
  # join with the Strep grup table for plotting
...
```{r}
ac_table <- fread("./06-publication/supplemental_tables/sup_table_anc_strep_clades_pct.tsv") %>%
  mutate(Full = "Calculus - Ancient")


strep_clades_pct %>%
  select(Full, Library_ID, Sanguinis, Mitis) %>%
  mutate(Full = as.character(Full)) %>%
  bind_rows(., ac_table %>%
              select(Full, Library_ID, Sanguinis, Mitis)) %>%
  rstatix::wilcox_test(., Mitis ~ Full) %>%
  full_join(., strep_clades_pct %>%
            select(Full, Library_ID, Sanguinis, Mitis) %>%
            mutate(Full = as.character(Full)) %>%
            bind_rows(., ac_table %>%
                      select(Full, Library_ID, Sanguinis, Mitis)) %>%
            rstatix::wilcox_effsize(., Mitis ~ Full)) %>%
  filter(str_detect(group1, "Calculus - Ancient") | str_detect(group2, "Calculus - Ancient"))
