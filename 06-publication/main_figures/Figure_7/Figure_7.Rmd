...
# ggsave("./06-publication/main_figures/Figure_7/strep_species_nhp.pdf", plot = strep_plots, device = "pdf",
...
nhp_strep_sp %>%
  ...
  left_join(., lib_metadata %>%
              select(Secondary_sample_accession, Host) %>%
              rename(Library_ID = 1)) %>%
  distinct() %>%
  select(Host, everything()) %>%
  fwrite(., "./06-publication/supplemental_tables/sup_table_nhp_strep_clades_pct.tsv", quote = F, sep = "\t")
